  ...
  SubShader
  {
    ...
    Pass // ind: 1, name: GlowPass
    {
      ...
      OUT_Data_Vert vert(appdata_t in_v)
      {
          ...
          if((_LightningTime.y<in_v.texcoord1.y))
          {
              u_xlatb0 = 1;
          }
          else
          ...
          u_xlat1.xyz = float3(((-in_v.texcoord1.xxz) + in_v.texcoord1.wyw));
          u_xlat6 = max(u_xlat1.z, 9.99999975E-06);
          u_xlat7.xy = float2(((-in_v.texcoord1.xz) + _LightningTime.yy));
          ...
          u_xlat2.xyz = float3((u_xlat1.yzw * in_v.color.xyz));
          u_xlat2.w = (u_xlat1.x * in_v.texcoord.w);
          out_v.color = u_xlat2;
          u_xlat0.x = dot(in_v.tangent.xz, in_v.tangent.xz);
          u_xlat0.x = rsqrt(u_xlat0.x);
          u_xlat0.xy = float2((u_xlat0.xx * in_v.tangent.xz));
          u_xlat0.xy = float2(((float2(u_xlat9, u_xlat9) * _TurbulenceVelocity.xz) + u_xlat0.xy));
          u_xlat6 = max(abs(in_v.tangent.w), 0.5);
          u_xlat6 = (_Turbulence / u_xlat6);
          u_xlat6 = (u_xlat6 * u_xlat9);
          u_xlat0.xz = (float2(u_xlat6, u_xlat6) * u_xlat0.xy);
          u_xlat1.xyz = float3((in_v.vertex.xyz * _LightningTime.xyz));
          ...
          u_xlat1.xy = float2((u_xlat1.xx * in_v.normal.xz));
          u_xlat7.x = (in_v.texcoord.x + (-0.5));
          u_xlat7.x = (u_xlat7.x + u_xlat7.x);
          u_xlat1.xy = float2((u_xlat7.xx * u_xlat1.xy));
          u_xlat7.x = (abs(in_v.tangent.w) + abs(in_v.tangent.w));
          u_xlat7.x = (u_xlat7.x * in_v.texcoord.z);
          u_xlat1.xy = float2((u_xlat7.xx * u_xlat1.xy));
          u_xlat1.xy = float2(((u_xlat1.xy * float2(1.5, 1.5)) + in_v.vertex.xz));
          u_xlat2.xy = float2((in_v.normal.zx * float2(-1, 1)));
          u_xlat2.xy = float2(normalize(u_xlat2.xy));
          u_xlat7.xy = float2((u_xlat7.xx * u_xlat2.xy));
          u_xlat2.x = (in_v.tangent.w / abs(in_v.tangent.w));
          u_xlat7.xy = float2((u_xlat7.xy * u_xlat2.xx));
