...
public class _2dxFX_AL_JellyAutoMove : MonoBehaviour
{
	...
	private void XUpdate()
	{
		...
		SaveMove1 = base.transform.position;
		if (SaveMove1.x != SaveMove2.x)
		{
			Heat += 0.5f;
		}
		if (SaveMove1.y != SaveMove2.y)
		{
			Heat += 0.5f;
		}
		if (SaveMove1.z != SaveMove2.z)
		{
			Heat += 0.5f;
		}
		...
		if (Heat < 0f)
		{
			RandomPos = UnityEngine.Random.Range(0, 256);
			Heat = 0f;
		}
		SaveMove2 = SaveMove1;
