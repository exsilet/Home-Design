...
namespace DigitalRuby.ThunderAndLightning
{
	public class LightningGenerator
	{
		...
		public void GenerateLightningBoltStandard(LightningBolt bolt, Vector3 start, Vector3 end, int generation, int totalGenerations, float offsetAmount, LightningBoltParameters p)
		{
			...
			List<LightningBoltSegment> segments = lightningBoltSegmentGroup.Segments;
			LightningBoltSegment item = new LightningBoltSegment
			{
				Start = start,
				End = end
			};
			...
			while (generation-- > 0)
			{
				...
				for (int i = startIndex; i < lightningBoltSegmentGroup.StartIndex; i++)
				{
					...
					a += result;
					List<LightningBoltSegment> segments2 = lightningBoltSegmentGroup.Segments;
					item = new LightningBoltSegment
					{
						Start = start,
						End = a
					};
					segments2.Add(item);
					List<LightningBoltSegment> segments3 = lightningBoltSegmentGroup.Segments;
					item = new LightningBoltSegment
					{
						Start = a,
						End = end
