...
public class _2dxFX_AL_GrassMultiFX : MonoBehaviour
{
	...
	private void XUpdate()
	{
		...
		if (CanvasSpriteRenderer != null)
		{
			...
			if (Wind != null)
			{
				CanvasSpriteRenderer.sharedMaterial.SetFloat("_Wind2", Wind.Evaluate(WindTime2));
			}
			if (Wind != null)
			{
				CanvasSpriteRenderer.sharedMaterial.SetFloat("_Wind3", Wind.Evaluate(WindTime3));
			}
			if (Wind != null)
			{
				CanvasSpriteRenderer.sharedMaterial.SetFloat("_Wind4", Wind.Evaluate(WindTime4));
			}
		...
		else if (CanvasImage != null)
		{
			...
			if (Wind != null)
			{
				CanvasImage.material.SetFloat("_Wind2", Wind.Evaluate(WindTime2));
			}
			if (Wind != null)
			{
				CanvasImage.material.SetFloat("_Wind3", Wind.Evaluate(WindTime3));
			}
			if (Wind != null)
			{
				CanvasImage.material.SetFloat("_Wind4", Wind.Evaluate(WindTime4));
			}
			CanvasImage.material.SetFloat("_Speed", Speed);
		}
		WindTime1 += Time.deltaTime / 8f * Speed;
		WindTime2 += Time.deltaTime / 8f * Speed;
		WindTime3 += Time.deltaTime / 8f * Speed;
		WindTime4 += Time.deltaTime / 8f * Speed;
	}
