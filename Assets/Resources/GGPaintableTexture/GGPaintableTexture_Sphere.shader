Shader "GGPaintableTexture/Sphere"
{
  ...
  SubShader
  {
    ...
    Pass // ind: 1, name: 
    {
      ...
      {
          OUT_Data_Vert out_v;
          u_xlat0.xy = float2((in_v.texcoord1.xy * _Channel.yy));
          u_xlat0.xy = float2(((in_v.texcoord.xy * _Channel.xx) + u_xlat0.xy));
          out_v.vertex.xy = float2(((u_xlat0.xy * float2(2, 2)) + float2(-1, (-1))));
      ...
      {
          OUT_Data_Frag out_f;
          u_xlat0_d.xyz = float3((in_f.texcoord1.xyz + (-_Position.xyz)));
