Shader "2DxFX/AL/SkyCloud"
{
  ...
  SubShader
  {
    ...
    Pass // ind: 1, name: FORWARD
    {
      ...
      {
          OUT_Data_Frag out_f;
          u_xlat0_d.xy = float2(((in_f.texcoord.xy * float2(float2(_Zoom, _Zoom))) + float2(_OffsetX, _OffsetY)));
          ...
          u_xlat0_d.xyz = float3((((-u_xlat0_d.xyz) * float3(_Intensity, _Intensity, _Intensity)) + u_xlat10_1.xyz));
    ...
    Pass // ind: 3, name: PREPASS
    {
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          OUT_Data_Frag out_f;
          u_xlat0_d.xy = float2(((in_f.texcoord.xy * float2(float2(_Zoom, _Zoom))) + float2(_OffsetX, _OffsetY)));
          u_xlat10_0 = tex2D(_MainTex2, u_xlat0_d.xy).w.x;
    ...
    Pass // ind: 6, name: ShadowCaster
    {
      ...
      {
          OUT_Data_Frag out_f;
          u_xlat0_d.xy = float2(((in_f.texcoord1.xy * float2(float2(_Zoom, _Zoom))) + float2(_OffsetX, _OffsetY)));
          u_xlat10_0 = tex2D(_MainTex2, u_xlat0_d.xy).w.x;
          u_xlat0_d.x = (u_xlat10_0 * in_f.texcoord3.w);
